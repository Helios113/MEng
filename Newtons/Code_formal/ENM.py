...
delta = 1e-6
conDelta = 0.001414  # maybe set this in a option too???

"""
Definition of different c configurations
"""
cConfig = {'2x': lambda x: 2*x, 'x+1': lambda x: x +
           1, '+1e-5': lambda x: x+1e-5, '-1e-5': lambda x: x-1e-5,
           '0.5x': lambda x: 0.5*x
           }


class Solver:
    def setup(self, method, func, span, config, rnge, res, show, save):
        """
        problem set up
        sets all parameters of the test
        """
        self.method = method
        self.func = func
        self.span = span
    ...
    def run(self):
        """
        runs the above defined test
        1. creates pool for execution
        2. creates result tensor of the correct shape (NxNx3)
        3. creates test parameters
        4. sets pool processes
        """
        res = self.res
