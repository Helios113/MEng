...
#norm = mpl.colors.Normalize(vmin=float(np.max(ans[..., -1])), vmax=float(np.max(ans[..., -1])))
for i in range(1,int(num)+1):
    ...
    if i == int(num):
        cb = fig.colorbar(cm.ScalarMappable(cmap=newcmp, norm=norm),cax=axins)
        cb.ax.set_ylabel('Iterations',)
    else:
        cb = fig.colorbar(cm.ScalarMappable(cmap=newcmp, norm=norm),ticks=[],cax=axins)
